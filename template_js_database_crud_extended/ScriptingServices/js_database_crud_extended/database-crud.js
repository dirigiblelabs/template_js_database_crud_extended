    ...
    if (!hasConflictingParameters(id, count, metadata, httpResponse)) {
        if (id) {
            ...
            if (entity !== null) {
                ...
                sendResponse(httpResponse, httpResponse.NOT_FOUND, 'text/plain', 'No entity found with \'#foreach ($tableColumn in $tableColumns)#if ($tableColumn.key == true)${tableColumn.name.toLowerCase()}#end#end\'=' + id);
            }
        } else if (count !== null) {
...
function handlePutRequest(httpRequest, httpResponse) {
    var entity = getRequestBody(httpRequest);
    var id = getIdParameter(httpRequest, xss);
    id = id !== null ? id : entity.#foreach ($tableColumn in $tableColumns)#if ($tableColumn.key == true)${tableColumn.name.toLowerCase()}#end#end;
    if (id !== null) {
        ...
        sendResponse(httpResponse, httpResponse.PRECONDITION_FAILED, 'text/plain', 'Expected \'#foreach ($tableColumn in $tableColumns)#if ($tableColumn.key == true)${tableColumn.name.toLowerCase()}#end#end\' parameter is missing!');
    }
...
function getIdParameter(httpRequest, xss) {
    ...
    id = id !== null ? id : xss.escapeSql(httpRequest.getParameter('#foreach ($tableColumn in $tableColumns)#if ($tableColumn.key == true)${tableColumn.name.toLowerCase()}#end#end'));
    return id;
}
